...
// ===== تنظیمات شما (بر حسب پیکسل واقعی تصویر) =====
const startX_px  = 20;   // شروع برش از لبه‌ی چپ (px)
const endX_px    = 350;  // پایان برش در محور افقی (px) — بخش راست حذف می‌شود
const startY_px  = 2;    // شروع برش از بالا (px)
const itemCount  = 100;  // تعداد تکه‌ها (ردیف‌ها)
// ==================================================
...
(async () => {
  ...
  // امن‌سازی مقادیر ورودی داخل محدوده‌ی تصویر
  ...
  if (cropWidth <= 0) {
    throw new Error(`cropWidth غیرمعتبر است: startX=${startX}, endX=${endX}, image.width=${image.width}`);
  }
  if (baseStep <= 0) {
    throw new Error(`ارتفاع هر برش صفر شد. startY=${startY}, image.height=${image.height}, itemCount=${itemCount}`);
  }
  ...
  for (let i = 0; i < itemCount; i++) {
    ...
    // برش از (startX, sy) با اندازه‌ی (cropWidth, thisHeight)
...
// ===== تنظیمات شما (بر حسب پیکسل واقعی تصویر) =====
const startX_px  = 2;   // شروع برش از لبه‌ی چپ (px)
const endX_px    = 250;  // پایان برش در محور افقی (px) — بخش راست حذف می‌شود
